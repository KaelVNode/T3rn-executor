...
while true; do
  ...
  case $pilihan in
    1)
      ...
      curl -L -o executor-linux-v0.32.0.tar.gz https://github.com/t3rn/executor-release/releases/download/v0.32.0/executor-linux-v0.32.0.tar.gz
      if [ $? -ne 0 ]; then
      ...
      if [ ! -f executor-linux-v0.32.0.tar.gz ]; then
        echo "File executor-linux-v0.32.0.tar.gz tidak ditemukan!"
        exit 1
      fi

      tar -xzvf executor-linux-v0.32.0.tar.gz
      if [ $? -ne 0 ]; then
        echo "Gagal mengekstrak file"
        exit 1
      fi
      rm -f executor-linux-v0.32.0.tar.gz
